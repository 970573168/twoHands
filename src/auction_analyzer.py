...
def update_record(table, item_id: str, fields: Dict):
    ...
    for k, v in fields.items():
        parts.append(f"{k} = :{k}"); values[f":{k}"] = _to_dynamo(v)
    for r in ("url", "ttl"):
        if r in fields: names[f"#{r}"] = r; parts = [p.replace(f"{r} =", f"#{r} =") for p in parts]
    table.update_item(Key={"itemID": str(item_id)}, UpdateExpression="SET "+", ".join(parts), ExpressionAttributeNames=names or None, ExpressionAttributeValues=values)
...
def check_limits():
    if _total_tokens >= MAX_TOKENS: raise RuntimeError(f"Token limit")
    e = 0 if _start_time is None else time.time()-_start_time
    if LAMBDA_TIMEOUT-e-TIMEOUT_BUFFER <= 0: raise RuntimeError("Timeout")
...
def call_ai(prompt: str) -> Tuple[Optional[Dict],Optional[str]]:
    ...
    for _ in range(3):
        ...
        for retry in range(AI_RETRIES):
            ...
            except Exception as e: logger.error(f"[{mode}] {type(e).__name__}")
...
def batch_parse(table, items: List[Dict], prompt_builder, batch_size: int) -> Dict:
    ...
    for start in range(0,len(items),batch_size):
        ...
        for mid in {str(i["itemID"]) for i in batch}-returned:
...
def scrape_closed(kw: str, cnt: int) -> List[str]:
    try:
        ...
        for item in items:
            try: update_record(closed_db, str(item["itemId"]), {"itemType":item.get("itemType",""),"title":item.get("title",""),"price":si(item.get("price",0)),"endTime":item.get("endTime") or "unknown","url":item.get("url",""),"sellerId":str(item.get("sellerId") or ""),"sellerRating":str(item.get("sellerRating") or ""),"detailDescription":item.get("detailDescription",""),"detailTitle":item.get("detailTitle",""),"detailScrapeStatus":item.get("detailScrapeStatus","NOT_SCRAPED")})
            except: pass
        return [str(i["itemId"]) for i in items]
    except Exception as e: logger.error(f"Closed scrape: {e}"); return []
...
def scrape_active(kw: str, cnt: int, min_p: int = 0) -> List[str]:
    try:
        ...
        if min_p>0: items = [i for i in items if si(i.get("price",0))>=min_p]
        items = items[:cnt]
        for item in items:
            try: update_record(active_db, str(item["itemId"]), {"itemType":item.get("itemType",""),"title":item.get("title",""),"price":si(item.get("price",0)),"buynowPrice":si(item.get("buynowPrice")) if item.get("buynowPrice") else None,"endTime":item.get("endTime") or "unknown","url":item.get("url",""),"sellerId":str(item.get("sellerId") or ""),"sellerRating":str(item.get("sellerRating") or ""),"detailDescription":item.get("detailDescription",""),"detailTitle":item.get("detailTitle",""),"detailScrapeStatus":item.get("detailScrapeStatus","NOT_SCRAPED")})
            except: pass
        return [str(i["itemId"]) for i in items]
    except Exception as e: logger.error(f"Active scrape: {e}"); return []
...
def execute_workflow(kw: str, ac: int, cc: int, force: bool) -> Dict:
    ...
    try:
        ...
        closed_items = [get_record(closed_db,cid) for cid in closed_ids if get_record(closed_db,cid)]
        closed_items = [i for i in closed_items if i and (force or i.get("modelStatus")==Status.PENDING)]
        if closed_items: batch_parse(closed_db, closed_items, build_parse_prompt, CLOSED_BATCH)
        ...
        active_items = [get_record(active_db,aid) for aid in active_ids if get_record(active_db,aid)]
        active_items = [i for i in active_items if i and (force or i.get("modelStatus")==Status.PENDING)]
        if active_items: batch_parse(active_db, active_items, build_parse_prompt, MODEL_BATCH)
        ...
        for aid in active_ids:
            try:
                ...
                if not item or item.get("modelStatus")!=Status.COMPLETED: continue
                if str(item.get("listingType","")).upper() in EXCLUDED_TYPES: continue
                
                ci, mi = find_comp(item, idx); stats = calc_stats(ci)
                pp = sd(item.get("price",0)); sh = get_shipping(item)
                bp = sd(item.get("buynowPrice")) if item.get("buynowPrice") else None
                
                pr = build_result(item, stats, pp, sh, bp, mi)
                rec = calc_decision(sd(pr.get("netProfitAtCurrentBid",0)),sd(pr.get("profitMarginAtCurrentBid",0)),sd(pr.get("roiAtCurrentBid",0)),sd(pr.get("pricingConfidence",0)),pr.get("riskLevel","HIGH"),si(pr.get("comparableCount",0)),si(item.get("missingParameterCount",0)),pr.get("pricingStatus",Status.INSUFFICIENT_DATA))
                save_pricing(aid, pr, rec)
            except RuntimeError: raise
            except Exception as e: logger.error(f"Pricing {aid}: {e}")
        
        result["status"] = "COMPLETED"; result["elapsed"] = round(time.time()-_start_time,1)
        return result
    except RuntimeError as e: return {**result,"status":"INTERRUPTED","reason":str(e)}
    except Exception as e: return {**result,"status":"FAILED","error":str(e)}
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if not kw: return {"statusCode":400,"body":json.dumps({"error":"keyword required"})}
