...
def norm(text: str) -> str:
    """标准化文本：全角转半角 + 去多余空格"""
    if not text: return ""
    t = str(text).strip().translate(str.maketrans(
...
def get_ai_cfg():
    ...
    for mode in order:
        if mode in _ai_state["failed_modes"]:
            if now - _ai_state["failed_modes"][mode] < _env("AI_COOLDOWN", 300, int):
                continue
    ...
    for _ in range(3):
        ...
        if is_gem:
            body = {
                ...
                "generationConfig": {"temperature": 0.0, "maxOutputTokens": cfg["max_tokens"]}
            }
        else:
            body = {
                "model": cfg["model"],
        ...
        headers = {"x-goog-api-key": cfg["key"], "Content-Type": "application/json"} if is_gem else {
        ...
        for retry in range(AI_RETRIES):
            try:
                ...
                _total_tokens += u.get("total_tokens", u.get("totalTokenCount", 0))
                ...
                logger.error(f"[{mode}] Attempt {retry+1}: {type(e).__name__}")
...
def save_model(table, item_id: str, parsed: Dict) -> str:
    ...
    logger.info(f"Model parsed: {item_id} -> {status} (listingType={lt}, condition={cond})")
    return status
...
def batch_parse(table, items: List[Dict], prompt_builder, batch_size: int) -> Dict:
    ...
    for start in range(0, len(items), batch_size):
        ...
        logger.info(f"AI parsing batch {start//batch_size+1}, size={len(batch)}")
        result, err = call_ai(prompt_builder(batch))
        
        if not result:
            logger.warning(f"AI returned empty: {err}")
            for item in batch:
...
def build_index(closed_ids: Set[str]) -> Dict[str, List[Dict]]:
    ...
    logger.info(f"Index built: {len(idx)} keys, excluded models: {excluded}")
    return idx
        ...
        if len(items) < MIN_COMPARABLE and ENABLE_FALLBACK:
            ...
            if fk:
                for ci in idx.get(f"FB:{fk}", []):
                    iid = str(ci.get("itemID", ""))
                    if iid and iid not in seen:
                        seen.add(iid); items.append(ci); fb += 1
            ...
            if fm:
                ...
                if b:
                    ...
                    for ci in idx.get(f"FAM:{fkey}", []):
                        iid = str(ci.get("itemID", ""))
                        if iid and iid not in seen:
                            seen.add(iid); items.append(ci); fam += 1
...
def calc_stats(items: List[Dict]) -> Dict:
    """计算价格统计"""
    records = [
        {"id": str(i.get("itemID", "")), "price": sd(i.get("price", 0))}
        for i in items if sd(i.get("price", 0)) > 0
    ]
    ...
    q1 = pct(prices, Decimal("0.25")); m = pct(prices, Decimal("0.5")); q3 = pct(prices, Decimal("0.75"))
    iqr = q3 - q1; lo = q1 - MAX_PRICE_DEV * iqr; hi = q3 + MAX_PRICE_DEV * iqr
    filtered = [r for r in records if lo <= r["price"] <= hi]
    fp = [r["price"] for r in filtered]; fn = len(fp)
    
    if fn < MIN_COMPARABLE:
        return {**base,
            "min": int(prices[0]), "max": int(prices[-1]),
            "q1": int(q1), "median": int(m), "q3": int(q3), "iqr": int(iqr),
            "lower": int(lo), "upper": int(hi),
            "f_count": fn, "suff": False,
            "reason": f"Outlier: {fn}<{MIN_COMPARABLE}",
            "f_median": int(m), "f_min": int(prices[0]), "f_max": int(prices[-1]),
            "f_prices": [int(p) for p in fp],
    ...
    return {**base,
        ...
        "q1": int(q1), "median": int(m), "q3": int(q3), "iqr": int(iqr),
        "lower": int(lo), "upper": int(hi),
        "f_count": fn, "excl": n - fn, "suff": True,
        "f_min": int(min(fp)), "f_max": int(max(fp)),
        "f_median": int(fmed),
        "f_avg": int((sum(fp) / Decimal(fn)).quantize(Decimal("1"))),
        "spread": spread,
        "f_prices": [int(p) for p in fp],
        "ids": [r["id"] for r in filtered]
    }
...
def calc_conf(stats: Dict, item: Dict = None, mi: Dict = None) -> Decimal:
    ...
    cc = stats["f_count"]; sr = stats.get("spread", Decimal("0"))
    tc = stats["count"]; ec = stats.get("excl", 0)
...
def calc_profit(est: Decimal, buy: Decimal, ship: Decimal) -> Dict:
    ...
    return {
        "net": net, "margin": margin, "roi": roi,
        "fee": fee, "repair": rep, "risk": risk, "tc": tc,
        "bep": max(0, int(bep)),
        "tp10": max(0, int((est * (Decimal("1") - FEE_RATE - REPAIR_RESERVE - RISK_RESERVE - Decimal("0.1")) - ship).quantize(Decimal("1")))),
        "tp20": max(0, int((est * (Decimal("1") - FEE_RATE - REPAIR_RESERVE - RISK_RESERVE - Decimal("0.2")) - ship).quantize(Decimal("1"))))
    }

def calc_decision(net: Decimal, margin: Decimal, roi: Decimal, conf: Decimal,
                  risk: str, cc: int, mc: int, status: str) -> str:
    """计算决策建议"""
    ...
    if margin >= BUY_MARGIN and risk in ("LOW", "MEDIUM") and conf >= Decimal("0.7"):
        return Recommendation.BUY
    if margin >= REVIEW_MARGIN and conf >= Decimal("0.5"):
        return Recommendation.REVIEW
    if roi >= MIN_ROI: return Recommendation.AI_RECHECK
    return Recommendation.NO

def build_result(item: Dict, stats: Dict, buy: Decimal, ship: Decimal,
                 buynow=None, mi=None) -> Dict:
    """构建定价结果"""
    ...
    dec = calc_decision(profit["net"], profit["margin"], profit["roi"], conf,
                        risk["level"], cc, mc, Status.COMPLETED)
...
# ======================================
# Workflow (核心修复: scrape 时初始化 modelStatus)
# ======================================

def scrape_closed(kw: str, cnt: int) -> List[str]:
    """抓取已结束商品，新商品自动设为 PENDING 状态"""
    try:
        ...
        for item in items:
            try:
                ...
                if existing:
                    # 已存在：仅更新基础信息，不覆盖 modelStatus / pricingStatus
                    update_record(closed_db, iid, {
                        "title": item.get("title", ""),
                        "price": si(item.get("price", 0)),
                    ...
                    update_record(closed_db, iid, {
                        ...
                        "detailTitle": item.get("detailTitle", ""),
                        "detailScrapeStatus": item.get("detailScrapeStatus", "NOT_SCRAPED"),
                        "modelStatus": Status.PENDING,       # ← 核心：标记为待解析
                        "pricingStatus": Status.NOT_RUN      # ← 标记为未定价
                    })
            except Exception as e:
                logger.error(f"Save closed item {item.get('itemId')}: {e}")
...
def scrape_active(kw: str, cnt: int, min_p: int = 0) -> List[str]:
    """抓取活跃商品，新商品自动设为 PENDING 状态"""
    try:
        ...
        for item in items:
            try:
                ...
                logger.error(f"Save active item {item.get('itemId')}: {e}")
...
def calc_min_price(closed_ids: List[str]) -> Dict:
    ...
    return {"min": max(1, int(sum(filtered) // len(filtered) * PRICE_MIN_RATIO))}
...
def execute_workflow(kw: str, ac: int, cc: int, force: bool) -> Dict:
    ...
    try:
        # Step 1: 抓取已结束商品
        check_limits()
        closed_ids = scrape_closed(kw, cc)
        result["closed"] = len(closed_ids)
        if not closed_ids:
            return {**result, "status": "NO_CLOSED"}
        
        # Step 2: AI 解析已结束商品（筛选 modelStatus == PENDING）
        closed_items = [
            get_record(closed_db, cid) for cid in closed_ids if get_record(closed_db, cid)
        ]
        closed_items = [
            i for i in closed_items if i and (force or i.get("modelStatus") == Status.PENDING)
        ]
        logger.info(f"Closed items to parse: {len(closed_items)} (pending)")
        if closed_items:
        ...
        # Step 4: 抓取活跃商品（使用最低价格过滤）
        active_ids = scrape_active(kw, ac, mp)
        result["active"] = len(active_ids)
        if not active_ids:
            return {**result, "status": "NO_ACTIVE"}
        
        # Step 5: AI 解析活跃商品（筛选 modelStatus == PENDING）
        active_items = [
            get_record(active_db, aid) for aid in active_ids if get_record(active_db, aid)
        ]
        active_items = [
            i for i in active_items if i and (force or i.get("modelStatus") == Status.PENDING)
        ]
        logger.info(f"Active items to parse: {len(active_items)} (pending)")
        if active_items:
        ...
        # Step 7: 对每个活跃商品进行定价（筛选 modelStatus == COMPLETED AND pricingStatus == PENDING）
        priced = 0
